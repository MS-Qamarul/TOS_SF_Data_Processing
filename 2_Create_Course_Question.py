...
def edit_excel_file(filename):
    ...
    # Insert "=AB1", "=AB2", ... into cells in column AF while AB is not empty
    row = 1
    while mappings_sheet[f'AB{row}'].value:
        mappings_sheet[f'AF{row}'] = f'=AB{row}'
        row += 1

    # Insert "Remapped Course Topic Name" into cell Z1
    mappings_sheet['Z1'] = 'Remapped Course Topic Name'

    # Insert VLOOKUP formula into cells in column Z while X is not empty
    row = 2
    while mappings_sheet[f'X{row}'].value:
        mappings_sheet[f'Z{row}'] = f'=VLOOKUP(TRIM(Y{row}), AD:AF, 3, FALSE)'
        row += 1

    # Create a new sheet called "4_Working"
    working_sheet = workbook.create_sheet("4_Working")

    # Copy values from columns A to K from "4. Course Question" sheet to "4_Working" sheet
    ...
    # Insert headers into cells L1, M1, N1, O1, and P1 in "4_Working" sheet
    ...
    # Insert formulas into columns L, M, N, and O in "4_Working" sheet
    row = 2
    ...
    # Create a new sheet called "4_CSV"
